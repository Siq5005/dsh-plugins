...
- **⏳ 待开发项目（D-020 采纳，下个版本回填式清单；实施顺序见决策 1，完成逐条回填到对应 DECISIONS 条目，先例 D-013）**：
  ...
  - **落地回填（2026-09-08，健壮性批次）**：A1/A3/A5 已实现并提交 main——`a190c75`（A1）、`5fcb828`（A5）、`6c82abf`（A3）；验证：JS 全套 27/27（含新增 5 项）、Python 单测 6/6、`helper.py --headless` 启动干净退出。**A3 桌面生效已完成**：用 venv Python 重建 helper 单文件并提交（`af05558`，47.3MB，重建后 headless ready/exit 0 + JS 27/27 复验）；PyInstaller 默认缓存目录在 workspace 外（`~/Library/Application Support/pyinstaller`）会被文件沙箱拒绝，构建时需 `PYINSTALLER_CONFIG_DIR` 指到 workspace 内（`build-helper/`，已被 .gitignore 覆盖）。插件重启（设置页关/开"启用大肥鱼"或重启 DSH）后桌宠才会加载新 helper。A1 的 import 期惰性加载与 A7 事件监听隔离尚未做（A7 与 A1 部分重叠，见清单）。
