...
- **⏳ 待开发项目（D-020 采纳，下个版本回填式清单；实施顺序见决策 1，完成逐条回填到对应 DECISIONS 条目，先例 D-013）**：
  ...
  - [x] **A7** 宿主总线/客户端故障隔离——session 监听与 client 槽注册各自包 guard（alpha.15）——✅ 2026-09-10，(a) `src/index.js` 两个总线监听体各自 try/catch（坏事件不再连坐其它订阅者，`test/listener-isolation.test.js` 2 项，含"隔离后正常事件仍送达 helper"）；(b) `lib/client.js` 卡片注册双层 guard（槽位契约变化只丢卡片不拖垮 WebUI，`test/client-guard.test.js` 3 项）
  - [x] **A8** pluginVersion 读 package.json——去掉硬编码 `'0.1.0'`（alpha.13）——✅ 2026-09-10，`src/index.js` 新增惰性 `pluginVersion()`（读 `package.json`，异常兜底 `'0.0.0'`，不进 import 期失败面），HELLO 改用它；`package.json` 版本随本批次 `0.1.0 → 0.2.0`；`test/plugin-version.test.js` 2 项（含假 helper 抓 HELLO 行的行为断言）
  - **落地回填（2026-09-10，A7/A8 批次）**：A7/A8 已实现并提交 main——`5f36008`（A8）、`35a5372`（A7a 总线隔离）、`86980b6`（A7b 客户端守卫）；验证：JS 全套 34/34（9 文件）、`node --check lib/client.js` 通过；纯 JS 改动，重启 DSH 即生效，无需重建 helper。README 验证状态同步更新（34 项 JS + 9 项 Python）。
  - **落地回填（2026-09-08，健壮性批次）**：A1/A3/A5 已实现并提交 main——`a190c75`（A1）、`5fcb828`（A5）、`6c82abf`（A3）；验证：JS 全套 27/27（含新增 5 项）、Python 单测 9/9（纯模型 6 + 真实 `PetWindow` 离屏接线 3，`QT_QPA_PLATFORM=offscreen` 不弹窗；无 PySide6 时自动跳过）、`helper.py --headless` 启动干净退出。**A3 桌面生效已完成**：用 venv Python 重建 helper 单文件并提交（`af05558`，47.3MB，重建后 headless ready/exit 0 + JS 27/27 复验），并用 PyInstaller PYZ 反查确认打包件内 `animation_model` 模块含 A3 符号（`drag_streak`/`drag_release`/`error_dizzy`）；PyInstaller 默认缓存目录在 workspace 外（`~/Library/Application Support/pyinstaller`）会被文件沙箱拒绝，构建时需 `PYINSTALLER_CONFIG_DIR` 指到 workspace 内（`build-helper/`，已被 .gitignore 覆盖）。**本地运行实为源码模式**：profile patch 的 `helper.command` 覆盖为 `~/.dsh-dafeiyu-venv/bin/python`（defaultCommand 被覆盖），故 A3 在插件重启后即生效，与打包二进制无关；已核实运行进程的 `animation_model.cpython-314.pyc` 与当前源码 mtime/size 一致。A1 的 import 期惰性加载仍为将来项（A7 的事件监听隔离已在本批完成）。
