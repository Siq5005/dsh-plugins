...
- **布局**：占用 shell 右侧列（对话区自动收缩、**不遮挡聊天**），顶部 tab 栏（文件/浏览器/Git）+ 底部面板（可调高度/隐藏）。入口：会话头部「工作台」按钮。
  - 该列在 0.1.2-rc.1 名为 **details**（`layout.openDetails/closeDetails`，槽名 `details`），上游 **0.1.5 起改名 rightbar**（`layout.openRightbar/closeRightbar`，槽名 `rightbar`），旧名被移除而非保留别名。本插件运行时探测布局服务暴露的方法，**同一份 client bundle 同时兼容两条线**。
...
- 已知限制：在 **≤0.1.2-rc.1** 上右侧列会**替换内置的「工具调用详情」面板**（VS Code 式右面板的取舍）；上游 **0.1.5 起已移除内置 Detail 面板**并换成新的 Sidebar/rightbar，该取舍随之消失——本插件成为该列的正规使用者，不再抢占内置面板。无文件 watcher（需手动刷新）；Git 无 push/pull/fetch。
