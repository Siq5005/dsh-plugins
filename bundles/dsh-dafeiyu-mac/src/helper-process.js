/**
 * HelperProcess：管理桌面渲染子进程（python3 helper.py）。
 *
 * 生命周期：spawn → READY 握手 → JSONL 双向通信 → PING/PONG 心跳 →
 * 异常退出自动重启（快照重放恢复状态）→ SHUTDOWN 优雅退出。
 * 设计参考 QCYTSN/dsh-dafeiyu (MIT)。
 */
...
export class HelperProcess {
  ...
  start() {
    ...
    child.once('spawn', () => {
      ...
      this.startupTimer = setTimeout(() => {
        if (this.child === child && !this.spawned) {
          this.logger.warn?.('dsh-dafeiyu-mac helper readiness timed out')
          child.kill()
        }
      }, startupTimeoutMs)
    ...
    createInterface({ input: child.stderr }).on('line', (line) => {
      if (line.trim()) this.logger.warn?.(`dsh-dafeiyu-mac helper: ${line}`)
    })
    return child
  }
  ...
  #handleReply(line) {
    ...
    this.logger.debug?.(`dsh-dafeiyu-mac helper: ${line}`)
  }
  ...
  #startHeartbeat() {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      if (Date.now() - this.lastPongAt > timeoutMs) {
        this.logger.warn?.('dsh-dafeiyu-mac helper heartbeat timed out')
        child.kill()
        return
      }
