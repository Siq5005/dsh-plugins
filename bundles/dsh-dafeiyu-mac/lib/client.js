// dsh-dafeiyu-mac WebUI 设置卡片（client 插件）。
// 通过 slots.inject('settings.plugin.item') 注入 DSH WebUI 设置页，
// 读写 Host 侧本地配置端点 /plugins/dsh-dafeiyu-mac/config。
// 机制参考 QCYTSN/dsh-dafeiyu (MIT) 的 client 插件写法。
window.__ModuleLoader__.load({ id: 'dsh-dafeiyu-mac', factory: (require) => {
  const module = { exports: {} }
  const exports = module.exports
  const React = require('react')
  const { useEffect, useRef, useState } = React
  const CONFIG_ENDPOINT = '/plugins/dsh-dafeiyu-mac/config'
  ...
  function BigFishCard() {
    ...
    return React.createElement('li', { style: cardStyle, 'data-testid': 'dsh-dafeiyu-mac-settings' },
      React.createElement('div', null,
        React.createElement('strong', { style: { fontSize: 16 } }, '大肥鱼桌面伴侣（macOS 复刻版）'),
        React.createElement('p', { style: { margin: '5px 0 0', opacity: 0.72 } },
          '由 DSH 会话状态驱动的桌面桌宠。入口与生命周期属于 DSH，窗口显示在桌面最上层。'),
      ),
      status === 'unavailable'
